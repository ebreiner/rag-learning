...
func TestTextNodeLabelCoverage(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			node := &step.Node{}
			err := textNode(node, rawTextItemForLabel(tc.label))
			if err != nil {
...
func TestTextNodeUnknownLabelDegradesGracefully(t *testing.T) {
	node := &step.Node{}
	err := textNode(node, rawTextItemForLabel("something_docling_invents_tomorrow"))
	if err != nil {
...
func TestGroupNodeLabelCoverage(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			node := &step.Node{}
			err := groupNode(node, rawGroupItemForLabel(tc.label))
			if err != nil {
...
func TestGroupNodeUnknownLabelDegradesGracefully(t *testing.T) {
	node := &step.Node{}
	err := groupNode(node, rawGroupItemForLabel("something_docling_invents_tomorrow"))
	if err != nil {
		t.Fatalf("unrecognized label should degrade to KindUnsupported, not error: %v", err)
	}
	if node.Kind != step.KindUnsupported {
...
func TestTableNodeLabelCoverage(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			node := &step.Node{}
			err := tableNode(node, rawTableItemForLabel(tc.label), heights)
			if err != nil {
...
func TestTableNodeUnknownLabelDegradesGracefully(t *testing.T) {
	node := &step.Node{}
	err := tableNode(node, rawTableItemForLabel("something_docling_invents_tomorrow"), pageHeightLookup{})
	if err != nil {
